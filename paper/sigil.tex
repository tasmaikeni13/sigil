...
\IfFileExists{tables/macros.tex}{\input{tables/macros}}{}
...
The detector's null distribution is not fitted. Every statistic \sigil{} reports
is a keyed Rademacher sum, so its tail is bounded exactly by Hoeffding's
inequality for every image, every finite carrier count, and no distributional
assumption at all; the searches the detector performs over anchors, scales,
...
dependence. The
false-positive rate is therefore a number the operator sets rather than a number
the experiment measures. The core of that argument --- the exact tail, the
...
log-magnitude of a canonical spectrum, which makes it exactly invariant to
translation, to any zero-phase radially symmetric filter, and to global gain, and
gives it high payload capacity at negligible cost. A learned stratum is trained
...
attack, so it reads through exactly the transformation that erases every linear
spectral mark. It is carried at two message-grid scales, which trade crop
...
Carriers in every stratum are bound to the image --- by content
descriptors and by a per-image nonce --- so an adversary who averages the
residuals of an arbitrarily large corpus recovers little. Measured as the attack
...
Evaluated on \sigilNImages{} images across \sigilNAttacks{} attacks --- valuemetric,
geometric, codebook and collusion, generative regeneration, white-box adaptive,
and stacked pipelines --- under an explicit attacker quality budget, \sigil{}
holds at a measured false-positive rate of $\sigilFPR\%$ while marking at
$\sigilPSNR$\,dB PSNR and $\sigilSSIM$ SSIM.
\end{abstract}
...
\]
holds exactly, for every image, at every finite carrier count, with no
...
is a detector whose false-positive rate is a parameter rather than a measurement
(\S\ref{sec:theory}).

The second half of the design is about what no single mechanism can do. A mark
that lives in a linear transform of the pixels can be made exactly invariant to
translation, to radially symmetric filtering, and to contrast --- but a
...
network is trained to read through such a round-trip survives it --- but the
network has no invariance anyone can prove and no natural exact null. \sigil{}
...
\item A watermark detector whose false-positive rate is exact, finite-sample and
      distribution-free, including the cost of its own synchronisation search
...
\item A machine-checked development in Lean~4 of the four results the security
      argument depends on: the exact null, the multiplicity charge, the fusion
...
\item An analytic stratum in the ring-normalised log-magnitude domain that is
      exactly invariant to translation, radially symmetric zero-phase filtering
      and global gain, and that resynchronises crops and reflections by
...
\item An evaluation protocol with an explicit attacker quality budget, a
      wrong-key false-positive control, and \sigilNAttacks{} attacks spanning six
...
A stratum produces a canonical-grid luminance residual; the residual is resampled
to $H\times W$ and added to the luminance channel, leaving chrominance untouched.
Because every residual is band-limited well below the canonical Nyquist rate the
resampling is faithful, and the immediate consequence is that a uniform rescale of
the delivered image maps back onto the same canonical representation. Scale
invariance is a property of the representation, not something the detector
searches for.
...
never of the keyed signs, so the exact null of \S\ref{sec:theory} is untouched.
...
\begin{theorem}[Exact null]
\label{thm:null}
Let $z\in\R^{K}$ be any evidence vector computed from the image, and let
...
\eqref{eq:stat} are exactly invariant to translation.
\end{theorem}
\begin{proof}
...
\begin{corollary}
Gaussian blur, unsharp masking, resampling roll-off, global contrast change and
the smooth radial envelope of a JPEG quantisation table are all of this form, and
the analytic statistic is invariant to the whole family at once.
\end{corollary}
...
\label{thm:endtoend}
 ...
 $\alpha$ for every image, every attack and every adversary.
\end{theorem}
\begin{proof}
...
\paragraph{Corpora.} Three, because the claim has three halves. A photographic
corpus of \arenaNImages{} high-resolution images tests the mark as a
signal-processing object across real content. A DIV2K split provides the
standard image-processing reference. A synthetic corpus generated locally with
SD-Turbo across a spread of subjects and styles tests the mark on the kind of
image it is actually meant to carry, and carries no third-party rights.
...
of a decibel of \sigil{}. On that footing the aggregate gap is small, $96.5\%$
against $95.1\%$ mean detection over admissible attacks, because most attacks
are survived comfortably by both. The aggregate is the wrong statistic. The
difference is not spread thinly across the suite; it is concentrated exactly
...
\midrule
\texttt{rsid\_codebook\_maximum} & 2\% & 100\% \\
\texttt{rsid\_stacked\_maximum} & 0\% & 94\% \\
\texttt{purify\_r2} & 0\% & 62\% \\
\texttt{img2img\_s0.3} & 21\% & 81\% \\
\texttt{vae\_sdxl\_n0.2} & 51\% & 99\% \\
\bottomrule
\end{tabular}
\end{center}
...
hold.}
\label{tab:headline}
\IfFileExists{tables/headline.tex}{\input{tables/headline}}{(pending)}
\end{table}
...
each is for.}
\label{tab:anchors}
\IfFileExists{tables/anchors.tex}{\input{tables/anchors}}{(pending)}
\end{table}
...
\IfFileExists{tables/descriptor.tex}{\input{tables/descriptor}}{(pending)}
\end{table}
...
\label{tab:attacks}
\IfFileExists{tables/attacks.tex}{\input{tables/attacks}}{(pending)}
\end{table*}
...
\label{tab:arena}
\IfFileExists{tables/arena.tex}{\input{tables/arena}}{(pending)}
\end{table}
...
\IfFileExists{tables/arena_families.tex}{\input{tables/arena_families}}{(pending)}
\end{table}
...
\label{tab:theory}
\IfFileExists{tables/theory.tex}{\input{tables/theory}}{(pending)}
\end{table}
...
the operating point drawn. Degradation is graceful in every family; where a
stratum crosses the line, the other is still above it.}
\label{fig:curves}
\end{figure*}
...
exactly the band a spectral mark occupies, motivate the stratification. What we
have not found elsewhere is the combination of an exact, search-corrected,
distribution-free null with a machine-checked proof of it, and content-derived
...
is not evidence. \sigil{} makes the false-positive rate a design parameter,
charges honestly for every search and every fusion it performs, and proves the
core of that argument in a proof assistant. Its robustness comes from carrying two
marks that fail on disjoint attacks and spending one budget across them, and its
resistance to cross-image aggregation comes from binding carriers to the image ---
including to a nonce that no amount of clustering by appearance can recover.
